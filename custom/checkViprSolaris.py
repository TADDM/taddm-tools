...
checkViprSolaris.jy - This script is used to check if hosts in ViPR_Solaris_hosts scope are being
...
and are physical Solaris so they are important for storage mapping, reporting, and 
billing. Ensure the following are true for each of the items:
   ...
   4) The 64-bit version of collectionengine is running and sudo is being used
      as configured in the discovery profile for HostStorageSensorSolaris
'''
dup_msg = '''\
  ...
  if data.next():
    ...
    if scope.hasElements():
      ...
      if len(notverified) > 0:
        ...
        print 'All physical Solaris hosts have been successfully discovered in the last 14 days.'
      if len(dups) > 0:
        print ''
        print dup_msg
        for dup in dups:
